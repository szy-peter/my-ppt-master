...
> **Global UI switch**: `CONFIRM_UI_ENABLED=false` (env, default `true`) disables the page entirely — the launch command exits non-zero with a `disabled via CONFIRM_UI_ENABLED=false` directive and **no** `result.json` is written. Treat that exactly like "the page never opened": run the staged confirmation in chat (below). `--shutdown` still runs for cleanup. See [`scripts/docs/confirm_ui.md`](scripts/docs/confirm_ui.md).
...
- **Global UI switch**: `LIVE_PREVIEW_ENABLED=false` (env, default `true`) skips the live preview entirely — the launch command exits non-zero with a `disabled via LIVE_PREVIEW_ENABLED=false` directive. That is an **intentional skip, not a launch failure**: do not troubleshoot it and do not block on a preview URL; just proceed with SVG generation and Step 7 export. `--shutdown` still runs for cleanup.
