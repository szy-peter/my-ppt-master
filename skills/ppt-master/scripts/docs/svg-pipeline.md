...
- Recorded narration is opt-in:
  - `notes_to_audio.py` uses the offline `sherpa` backend by default (intranet; needs `SHERPA_TTS_SERVER_URL`), or `edge-tts` / a configured cloud TTS provider (`elevenlabs`, `minimax`, `qwen`, `cosyvoice`), and generates one audio file per slide into `audio/`
