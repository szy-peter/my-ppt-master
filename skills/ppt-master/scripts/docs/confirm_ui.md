...
- **Global UI switch** `CONFIRM_UI_ENABLED` (env, default `true`): set `false` and every launch / wait mode short-circuits — the command logs `disabled via CONFIRM_UI_ENABLED=false — use the chat confirmation path (SKILL.md Step 4).` and exits non-zero (`3`), so the caller drops to the chat confirmation path with no `result.json` written. Read from the shared `.env` via the `CONFIRM_UI_` prefix (same resolution order as the image / audio keys). `--shutdown` is exempt, so Step 4 cleanup still runs on every path.
